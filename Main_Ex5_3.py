...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, params):
        ...
        self.length_scale = 1
        self.micro_size = 0.3*self.length_scale #0.23 0.33 0.5
        ...
        self.mesh_args = { "mesh_size_frac": 2*self.mesh_size/3, "mesh_size_min": 1 * self.mesh_size, "mesh_size_bound": 12* self.mesh_size} 
        ...
        xcen =  1.2
        ycen =  0.8
        fracture2 = np.array([[xcen - 0.7*lenfra*np.cos(phi), ycen - 0.7*lenfra*np.sin(phi)],
                              [xcen + 0.7*lenfra*np.cos(phi), ycen + 0.7*lenfra*np.sin(phi)]]) 
        
        phi = 3.8*np.pi/4
        xcen =  0.8
        ycen =  1.2
        fracture3 = np.array([[xcen - lenfra/2*np.cos(phi), ycen - lenfra/2*np.sin(phi)],
                              [xcen + lenfra/2*np.cos(phi), ycen + lenfra/2*np.sin(phi)]]) 
    ...
    def after_newton_convergence(self, solution, errors, iteration_counter):
        ...
        if len(self.fracture) == 3:
            tips_actualy = tips
                
        elif len(self.fracture) == 2:
    ...
    def propagation_small_scale(self, fracture_small, newfrac, tips, disp_cells, pres_cells, QPE):    
        ...
        for i in range(len(tips_small)):
            ...
            if frac_box:        
                ...
                mesh_args = { "mesh_size_frac": self.mesh_size_micro, "mesh_size_min": 2 * self.mesh_size_micro, "mesh_size_bound": 8 * self.mesh_size_micro}
...
k = 0
while setup.time <= setup.end_time and setup.tips[0,0] >= 0.6:
