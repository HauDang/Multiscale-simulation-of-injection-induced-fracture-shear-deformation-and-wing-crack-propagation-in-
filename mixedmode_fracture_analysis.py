...
def NN_recovery( values, p, t):
    ...
    # points = (p[t[:,0],:] + p[t[:,1],:] + p[t[:,2],:])/3
    
    # valnod = inter.griddata(points, values, p, method = 'cubic')
...
def adjustmesh(g, tips, gap):
    ...
    poiind = []
    # if len(grid_0d) > 0:
    #     for i in range(len(grid_0d)):
    #         poii = grid_0d[i,:]
    #         dis2poi = np.sqrt((p[:,0] - poii[0])**2 + (p[:,1] - poii[1])**2)
    #         poiind = np.where(dis2poi == np.min(dis2poi))[0]
    ...
    if tips is not None:
        for i in range(tips.shape[0]):
            ...
            if len(tipind) == 2:
                ...
                # face1 = np.setdiff1d(np.where(face_ind == tipind[0])[0],frac_fac)
                # face2 = np.setdiff1d(np.where(face_ind == tipind[1])[0],frac_fac)
        ...
        for i in range(len(frac_fac)):
            ...
            node_adj = np.setdiff1d(node_adj, poiind)
            p[node_adj,:] = p[node_adj,:] - normi*gap/2
            node_adj_old = np.concatenate((node_adj_old,node_adj))
        fra_ind1 = face_ind[frac_fac,0]
        fra_ind2 = face_ind[frac_fac,1]
        for i in range(len(poiind)):
            idi = np.append(np.where(poiind[i] == fra_ind1)[0], np.where(poiind[i] == fra_ind2)[0])
            fra_fac_con = face_ind[frac_fac[idi],:]
            nod_con = np.setdiff1d(np.unique(fra_fac_con),poiind[i])
            nor_vec = (p[nod_con[0],:] +  p[nod_con[1],:])/2 - p[poiind[i],:] 
            nor_vec = nor_vec/np.sqrt(sum(nor_vec**2))
            p[poiind[i],:] = p[poiind[i],:] + nor_vec*gap/2*np.sqrt(2)/2
            
        if len(tips_on_bou) == 2:
            if p0[tips_on_bou[0],0] == np.min(p0[:,0]) or p0[tips_on_bou[0],0] == np.max(p0[:,0]):
                p[tips_on_bou[0],0] = p0[tips_on_bou[0],0]
                
            if p0[tips_on_bou[0],1] == np.min(p0[:,1]) or p0[tips_on_bou[0],1] == np.max(p0[:,1]):
                p[tips_on_bou[0],1] = p0[tips_on_bou[0],1]
                
            if p0[tips_on_bou[1],0] == np.min(p0[:,0]) or p0[tips_on_bou[1],0] == np.max(p0[:,0]):
                p[tips_on_bou[1],0] = p0[tips_on_bou[1],0]
                
            if p0[tips_on_bou[1],1] == np.min(p0[:,1]) or p0[tips_on_bou[1],1] == np.max(p0[:,1]):
                p[tips_on_bou[1],1] = p0[tips_on_bou[1],1] 
...
def trisurf( p, t, fn = None, point = None, value = None, vmin = None, vmax = None, infor = None):
    ...
    if t.shape[1] == 3:
        ...
        im = grid.plot(X, Y, 'b-', linewidth = 0.1)
    ...
    plt.show()   
    # grid.set_xlim(0.2, 0.7)
    # grid.set_ylim(0.4, 0.6)
    # plt.axis('off')
def plot( p ):
    '''
    Plot a triangular mesh, points, represent value at each node by color 
...
def refinement( p, t, p0, t0, fracture, tips, A0, lmin, gap):
    ...
    if len(eleref1) > 0:
        ...
        eleref1 = eleref1[np.where(areele1 >= 4.0*A0)[0]]
    ...
    if len(eleref2) > 0:
        ...
        eleref2 = eleref2[np.where(areele2 >= 7.0*A0)[0]]   
...
def remesh_at_tip(gb, p_ref, t_ref, fracture, lmin, newfrac, gap):
    # import porepy as pp
    ...
    p_bef, t_bef = p_ref, t_ref #adjustmesh(g2d, tips, gap) 
    ...
    p, t, fn, cf, iniang = do_remesh(p_ref, t_ref, lmin, fracture, gap, newfrac)
    # pts = np.zeros(shape = (p.shape[0], 3))
    # pts[:,0] = p[:,0]; pts[:,1] = p[:,1]
    # g_2d: pp.Grid = pp.TriangleGrid(pts.transpose(), t.transpose())
    # grids = [[g_2d], g1d, []]
    # gb0 = pp.meshing.grid_list_to_grid_bucket(grids)
    ...
    for j in range(len(faces_on_surface)):
        for i in range(len(faces_on_surface[j])):
            # print(i)
            # dis = np.sqrt((fc_frac_bef[j][i,0] - face_center_modified[:,0])**2 +
            #               (fc_frac_bef[j][i,1] - face_center_modified[:,1])**2)
            # faces_frac_aft = np.where( dis == np.min(dis) )[0]
    ...
    for j in range(len(face_oneside_index)):
        ...
        for i in range(len(face_oneside_index[j])):
            # dis = np.sqrt((face_oneside_bef[j][i,0] - face_center_modified[:,0])**2 +
            #               (face_oneside_bef[j][i,1] - face_center_modified[:,1])**2)
            # face_oneside_index_aftj[i] = np.where( dis == np.min(dis) )[0]
...
def do_remesh(p, t, lmin, fracture, gap, newfrac = None):
    ...
    if len(fracture) == 2:
        ...
        tips_actualy = tips.copy()
        for i, tipi in enumerate(tips):
            disx = np.min([np.abs(tipi[0] - np.min(p[:,0])), np.abs(tipi[0] - np.max(p[:,0]))])
            disy = np.min([np.abs(tipi[1] - np.min(p[:,1])), np.abs(tipi[1] - np.max(p[:,1]))])
            if disx < gap or disy < gap: 
                tips_actualy = np.delete(tips_actualy,i, axis = 0)
    else:
        tips_actualy = tips
    iniang = []
    if newfrac is None:
...
def rosette_element(p, t, fracture, tip, r, gap ):
    ...
    alpha =  np.array([i*(2*np.pi - angle0)/n + angle0/2 for i in range(n+1)])
    ...
    # trisurf( p, t, fn = None, point = prose, value = None, vmin = None, vmax = None, infor = None)
    id1 = np.arange(0,n) + 1
    ...
    r0 = alpha*0 + r*1.7; r_control = r2*1.2; 
    r0[0] = r_control; r0[-1] = r_control 
    r0[1] = r_control; r0[-2] = r_control
    # r0[2] = r_control; r0[-3] = r_control# This based on experiments
    xin = -r0*np.cos(alpha + thetha) + tip[0]
    yin = -r0*np.sin(alpha + thetha) + tip[1]
    pvout = np.concatenate((xin.reshape(len(xin),1), yin.reshape(len(xin),1)), axis=1)
    # trisurf( p, t, fn = None, point = pvout, value = None, vmin = None, vmax = None, infor = None)
    isinside = inpolygon(p, pvout)
    ...
    # trisurf( p, t, fn = None, point = p[nodoutdel,:], value = None, vmin = None, vmax = None, infor = 1)
    while len(nodoutdel) > 0:
        ...
        if len(indP1) == 0:
            indP1 = np.intersect1d(nodex, np.setdiff1d(indfra, indtip))
    ...
    if len(np.intersect1d(np.where(t == indoutdel[-2])[0], np.where(t == indoutdel[-1])[0])) == 0:
        index1 = np.unique(t[np.where(t == indoutdel[-2])[0],:])
        index2 = np.unique(t[np.where(t == indoutdel[-1])[0],:])
        index = np.intersect1d(np.intersect1d(index1, index2), indfra)
        indoutdel = np.append(np.append(indoutdel[0:-1], index), indoutdel[-1])
    ...
    pvnew = np.concatenate((pv1,pv2,pv3), axis = 0)
    inddel = np.where(np.sqrt(np.sum((pvnew[1:] - pvnew[:-1])**2, axis = 1)) < TOL)[0]
    pvnew = np.delete(pvnew,inddel,axis = 0) 
    # plot(pvnew)
    # generate a grid bewtween elements's tip and pv   
    ptran, ttran = fillgap(pvnew) 
...
def smoothing(p, t, nodfix):
    ...
    return p, t
def splitelement( p, t, fracture, newfrac, gap, solution = None ):
    for i in range(len(newfrac)):
        tip = newfrac[i][0,:]
        tipnew = newfrac[i][1,:]
        indfra = []
        for j in range(len(fracture)):
            ds = p2segment(p, fracture[j])
            ind = np.where(ds < gap + TOL)[0]
            indfra = np.append(indfra, ind)
        indfra = np.int32( np.unique(indfra) )
        ...
        dis2tip = np.sqrt((p[:,0] - tipnew[0])**2 + (p[:,1] - tipnew[1])**2)
        indtipnew = np.where( dis2tip == np.min(dis2tip) )[0]
        ...
        elearo = np.where(t == b1)[0]
        indout = t[elearo,:]; indout = np.unique(indout)
        indout = np.setdiff1d(indout,[bb,bt,b1])
        indouti = np.array([bb])
        A0 = (p[bb,:] +  p[bt,:])/2
        A1 = p[b2,:]
        A2 = p[indout,:]
        d0 = (A1[0] - A0[0])*(A2[:,1] - A0[1]) - (A1[1] - A0[1])*(A2[:,0] - A0[0])                
        posnod = np.where(d0 <= 0)[0]
        bb = indout[posnod]
        indouti = np.append(bb,indouti[-1])  
        
        sumind = np.array(ismember(t[elearo,0],indouti)) + np.array(ismember(t[elearo,1],indouti)) + np.array(ismember(t[elearo,2],indouti))
        group1 = elearo[np.where(sumind == 2)[0]]
        p0 = (pb + pt)/2
        vt1 = tip - p0
        nt1 = np.array(([-vt1[1], vt1[0]]))/norm(vt1)
        
        vt2 = tipnew - p0
        nt2 = np.array(([-vt2[1], vt2[0]]))/norm(vt2)
        
        nt = nt1 + nt2; nt = nt/norm(nt)
        pbnew = tip - nt*gap/2
        ptnew = tip + nt*gap/2
        totnod = p.shape[0]
        p[b1,0] = ptnew[0]; p[b1,1] = ptnew[1]
        p = np.concatenate((p,pbnew.reshape(1,2)),axis = 0)
        for j in range(len(group1)):
            idecha = np.where(t[group1[j],:] == b1)[0]
            t[group1[j],idecha] = totnod
        if solution is not None:
            solnew = solution[b1,:]
            solution = np.concatenate((solution,solnew.reshape(1,2)),axis = 0)
...
def solution(material, p, t, qpe, p1, t1, dispnod, presnod, initial_fracture, gap, QPE):
    ...
    for i in range(len(initial_fracture)):
        ...
        ind = np.where(ds < gap + TOL)[0]
        indfra = np.append(indfra, ind)
    indfra = np.int32( np.unique(indfra) ) #; indfra = indfra[ indfra <= num_nodes] 
    
    remove_indfra = []
    for _, qpei in enumerate(qpe):
        # remove_indfra = np.append(remove_indfra, np.unique(t[qpei,:]) ) #t[qpei[0],0]
        remove_indfra = np.append(remove_indfra, np.array([t[qpei[0],1], t[qpei[-1],5] ]) ) #t[qpei[0],0]
        # remove_indfra = np.append(remove_indfra, np.array([t[qpei[0],1], t[qpei[0],0], t[qpei[-1],5] ]) ) #
    remove_indfra = np.int32(remove_indfra)
    
    indfra = np.setdiff1d(indfra, remove_indfra)
    topind = np.setdiff1d(p2index(p,nodaro[[3,2],:],gap), indfra)#[ p2index(p,nodaro[[3,2],:]) <= num_nodes] 
    botind = np.setdiff1d(p2index(p,nodaro[[0,1],:],gap), indfra)#[ p2index(p,nodaro[[0,1],:]) <= num_nodes] 
    lefind = np.setdiff1d(p2index(p,nodaro[[0,3],:],gap), indfra)#[ p2index(p,nodaro[[0,3],:]) <= num_nodes]   
    rigind = np.setdiff1d(p2index(p,nodaro[[1,2],:],gap), indfra)#[ p2index(p,nodaro[[1,2],:]) <= num_nodes] 
...
def FEM_solution(material, p, t, qpe, bc_type, bc_value):
    ...
    righs = loadtraction(p, t, tradof = bc_type['neu'], traval = bc_value['neu'])
    disp = linearsolver(lefhs, righs, bc_type['dir'], bc_value['dir'], p.shape[0]*2)
    return disp
def t3tot6(p, t, tips = None, QPE = None):
...
def SIF(p6, t6, disp, young, poisson, qpe, QPE ):
    ...
    for i in range(len(qpe)):
        ...
        if t6.shape[1] == 6:
            ...
            if QPE:
                ...
                K1 = young/2/(1+k)/(1+poisson)*np.sqrt(2*np.pi/L)*(pc[1] - pe[1])
                K2 = young/2/(1+k)/(1+poisson)*np.sqrt(2*np.pi/L)*(pc[0] - pe[0])
        if t6.shape[1] == 3:
            e = t6[qpei[0],1]
            a = t6[qpei[0],0]
            c = t6[qpei[-1],2]
        ...
        if d2s1 >= 0 and d2s2 >= 0:
            ...
            # thetha = np.sign(ang1)*abs(ang1)
            thetha = np.sign(ang1)*min(90*np.pi/180,abs(ang1))
        else:
            # thetha = np.sign(ang2)*abs(ang2)
            thetha = np.sign(ang2)*min(60*np.pi/180,abs(ang2))
