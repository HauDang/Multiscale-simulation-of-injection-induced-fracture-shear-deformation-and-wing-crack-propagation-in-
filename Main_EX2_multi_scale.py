...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, box, fracture, mesh_args, params):
        ...
        # Time seconds
        self.time = 0 
        self.end_time = 30*24*60*60
        self.time_step = 0.1*60*60   
        self.time_step_fix = 0.1*60*60 
    ...
    def set_rock_and_fluid(self):
        ...
        self.material = dict([('YOUNG', self.YOUNG), ('POISSON', self.POISSON), ('KIC', 1e6) ])      
    ...
    def _source_scalar(self, g):
        ...
        if g.dim == 1: #and self.pro_cri == False:
            print('injection')
            inject = (g.cell_centers[0,:] <= 5.2) * (g.cell_centers[0,:] >= 4.8) * (g.cell_centers[1,:] <= 5.01) * (g.cell_centers[1,:] >= 4.99)
            values[inject] = g.cell_volumes[inject]*self.INJECTION*self.time_step
        return values
    ...
    def _bc_values_mechanics(self, g):
        ...
        # values[0, north] = -2e3*g.face_areas[north]
    ...
    def after_newton_convergence(self, solution, errors, iteration_counter):
        ...
        keq, newfrac, tips0 = self.propagation_small_scale( tips, disp_cells )
        ...
        if self.pro_cri:
            self.lenfra.append(self.min_face)
        else:
    ...
    def propagation_small_scale(self, tips, disp_cells):
        ...
        for i in range(len(tips)):
            tipi = tips[i]
            box_small = {"xmin": tipi[0] - self.min_face*10, 
                         "ymin": tipi[1] - self.min_face*10, 
                         "xmax": tipi[0] + self.min_face*10, 
                         "ymax": tipi[1] + self.min_face*10}  
            ...
            for j, fracturej in enumerate(self.fracture):
                ...
                if frac_box:        
                    ...
                    Gi, keqi, craangi, iniangi = analysis.evaluate_propagation_small(self.material, p_small, t_small, frac_small, tipi, 
...
mesh_size = 0.1
mesh_args = { "mesh_size_frac": mesh_size, "mesh_size_min": 1 * mesh_size, "mesh_size_bound": 10 * mesh_size } 
params = {"folder_name": "biot_2","convergence_tol": 2e-7,"max_iterations": 20,"file_name": "main_run",}


box = {"xmin": 0, "ymin": 0, "xmax": 10, "ymax": 10}  
fracture1 = np.array([[4.8, 5.0],[5.2, 5.0]])   
# fracture2 = np.array([[2.3, 3.0],[2.5, 3.0]])   
...
# t = setup.t
# face = setup.fa_no
# analysis.trisurf(  self.p, self.t, fn = None, point = tips0, value = None, infor = 1)
...
analysis.trisurf( p + disp_nodes*1e2, t, fn = None, point = None, value = setup.stored_pres[k], infor = None)

# analysis.trisurf( p + disp_nodes*1e3, t, fn = None, point = None, value = setup.stored_disp[k][:,1], infor = None)
