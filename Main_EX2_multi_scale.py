...
class ModelSetup(pp.ContactMechanicsBiot, pp.ConformingFracturePropagation):
    def __init__(self, params):
        ...
        self.length_scale = 1
        self.micro_size = 0.6*self.length_scale #0.23 0.33 0.5
        
        # ok
        nn = 2
        self.mesh_size = 0.005*self.length_scale*nn
        self.mesh_size_micro = self.mesh_size
        self.mesh_args = { "mesh_size_frac": self.mesh_size, "mesh_size_min": 1 * self.mesh_size, "mesh_size_bound": 12* self.mesh_size} 
        ...
        self.initial_aperture = 1.000e-3*self.length_scale
        
        # Time seconds       
        self.end_time = 10000*60
        self.inject_time = [0*20, 10000*60]
        self.time_step = 60*20/1
        self.time_step_fix = 60*20/1
    ...
    def propagation_small_scale(self, tips, disp_cells, QPE):
        ...
        for i in range(len(tips)):
            ...
            area_mic = self.micro_size**2
            
            area_mac = (self.box["xmax"]  -  self.box["xmin"])*(self.box["ymax"]  -  self.box["ymin"])
            
            ratio = 1 #1/8*(area_mic/area_mac) + 7/8
            ...
            for j, fracturej in enumerate(self.fracture):
                ...
                if frac_box:        
                    ...
                    mesh_args = { "mesh_size_frac": self.mesh_size_micro, "mesh_size_min": 2 * self.mesh_size_micro, "mesh_size_bound": 4 * self.mesh_size_micro}
...
k = 0
while setup.time < setup.end_time and np.sum(setup.lenfra) <= 0.25:
